...
if [ "${SPLIT_DONE}" = "true" ]; then
    ...
    SONNET_BATCH_COUNT=$(ls "${WORKDIR}/batches/sonnet"/batch_[0-9][0-9][0-9].csv 2>/dev/null | wc -l | tr -d ' ')
...
SONNET_BATCHES=$(ls "${WORKDIR}/batches/sonnet"/batch_[0-9][0-9][0-9].csv 2>/dev/null | sort)
...
decisions = Counter((r.get('decision') or '').lower() for r in rows)
confidence = Counter((r.get('confidence') or '').lower() for r in rows)
...
# Load original queue for title/abstract + source_queue lookup
queue = {r['item_key']: r for r in
         csv.DictReader(open('${QUEUE_RANDOM}', encoding='utf-8-sig'))}

opus_rows = []
apply_rows = []
promoted_maybe = []   # maybe_queue items Sonnet said keep — need case-by-case

for row in sonnet:
    decision   = (row.get('decision')   or '').lower().strip()
    confidence = (row.get('confidence') or '').lower().strip()
    orig       = queue.get(row['item_key'], {})
    source_q   = orig.get('source_queue', 'keep_queue')
    ...
    # maybe_queue + keep = promotion candidate — route to Phase 2 02-Maybe
    # for case-by-case human review before final promotion to 01-Keep
    elif decision == 'keep' and source_q == 'maybe_queue':
        promoted_maybe.append({
            'item_key':   row['item_key'],
            'decision':   'maybe',         # downgrade to maybe for Zotero placement
            'confidence': confidence,
            'rationale':  row.get('rationale', ''),
            'note':       'promoted_from_phase1_maybe',
        })
...
with open('${OPUS_INPUT}', 'w', newline='', encoding='utf-8') as f:
    fieldnames = ['item_key','title','abstract',
                  'prior_decision','prior_confidence','prior_rationale','source_queue']
    w = csv.DictWriter(f, fieldnames=fieldnames)
    w.writeheader()
    w.writerows(opus_rows)

# Write apply-from-sonnet (keep_queue keeps + both_queue keeps + all discards)
...
# Write promoted-maybe (maybe_queue items Sonnet said keep)
with open('${WORKDIR}/promoted_maybe.csv', 'w', newline='', encoding='utf-8') as f:
    fieldnames = ['item_key','decision','confidence','rationale','note']
    w = csv.DictWriter(f, fieldnames=fieldnames, extrasaction='ignore')
    w.writeheader()
    w.writerows(promoted_maybe)

# Summary
src_counts = Counter(queue.get(r['item_key'],{}).get('source_queue','?') for r in sonnet)
print(f'Source queue breakdown: {dict(src_counts)}')
print(f'Routed to Opus:         {len(opus_rows)}')
print(f'Direct apply:           {len(apply_rows)}')
print(f'Promoted maybe (phase1-maybe + sonnet-keep): {len(promoted_maybe)}')
"
...
if [ "${OPUS_SPLIT_DONE}" = "true" ]; then
    ...
    if [ "${OPUS_COUNT}" -gt 0 ]; then
        ...
        OPUS_BATCH_COUNT=$(ls "${WORKDIR}/batches/opus"/batch_[0-9][0-9][0-9].csv 2>/dev/null | wc -l | tr -d ' ')
...
OPUS_BATCHES=$(ls "${WORKDIR}/batches/opus"/batch_[0-9][0-9][0-9].csv 2>/dev/null | sort || true)
...
if rows:
    ...
    from collections import Counter
    print(f'Decisions: {dict(Counter((r.get(\"decision\") or \"\").lower() for r in rows))}')
    print(f'Confidence: {dict(Counter((r.get(\"confidence\") or \"\").lower() for r in rows))}')
else:
...
if [ "${APPLY_DONE}" = "true" ]; then
    ...
    # Merge Sonnet + Opus + promoted_maybe apply queues
    # Priority: Opus > promoted_maybe > Sonnet
...
promoted = list(csv.DictReader(open('${WORKDIR}/promoted_maybe.csv', encoding='utf-8-sig'))) \
           if os.path.exists('${WORKDIR}/promoted_maybe.csv') else []

# Build merged dict — higher priority sources override lower
all_rows = {r['item_key']: r for r in sonnet}
for r in promoted:
    all_rows[r['item_key']] = r   # promoted_maybe overrides sonnet
for r in opus:
    all_rows[r['item_key']] = r   # Opus overrides all
...
print(f'  keep={dec[\"keep\"]} maybe={dec[\"maybe\"]} discard={dec[\"discard\"]}')
print(f'  (maybe = phase1-maybe items promoted by Sonnet keep — go to Phase2 02-Maybe)')
print(f'  Sonnet direct: {len(sonnet)}  Promoted maybe: {len(promoted)}  Opus: {len(opus)}')
"
...
def count_decisions(f):
    try:
        ...
        return Counter((r.get('decision') or '').lower() for r in rows), len(rows)
    except:
        return Counter(), 0
