...
CLAUDE_MODEL = "sonnet"  # 'sonnet' / 'opus' / 'haiku' — claude CLI accepts shortnames
...
def main():
    ...
    # Append PID so we can identify and kill stale instances
    ...
    for i, k in enumerate(state["all_keys"]):
        ...
        # Log ALL decisions to CSV with full abstract
        if d_val == "discard":
            state["discarded"] += 1
        elif d_val == "maybe":
            state["maybe"] += 1
